...
//export default function EstructuraInicio() 
{
    ...
    const canvasRef = useRef(null);    const [selectedArea, setSelectedArea] = useState(null);    const [imageSize, setImageSize] = useState({ width: 0, height: 0 });
    ...
    useEffect(() => {
        if (loadedImage) {
            const ctx = canvasRef.current.getContext('2d');ctx.clearRect(0, 0, canvasRef.current.width, canvasRef.current.height);            ctx.drawImage(loadedImage, 0, 0); drawAllAreas(ctx);   drawHighlight(ctx);        }    }, [selectedArea, loadedImage, drawAllAreas, drawHighlight]);
            ...
            .then((json) => {
                ...
                if (obj) {
                    setFormData({
                        ...
                        estado: obj.estado,
                        coordenadas: obj.coordenadas,                        color: obj.color                    });                    setMostrarResultados(true);                }
            })
            .catch((error) => {
